    ...
    <b-navbar :class="`layout-nav-bar  `">
      <b-navbar-nav class="body-left">
        <b-nav-item link-classes="p-0">
          <div class="logo d-flex flex-nowrap align-items-center overflow-hidden ">
            ...
            <span class="brand collapse-hidden">海·星の博客</span>
          </div>
        </b-nav-item>
      </b-navbar-nav>
      <b-navbar-nav class="body-right dropdown">
        <b-nav-item class="nav-title  ">
          <a @click="switchMenu">
            <b-icon :icon="isLeftIcon?'text-indent-left':'text-indent-right'" scale="2" />
          </a>
        </b-nav-item>
        ...
        <b-nav-item>
          <b-breadcrumb>
            ...
            <b-breadcrumb-item :id="`bc-nav-item-${index}`" :key="r.name+index" v-for="(r,index) of $route.matched">
              {{r.name }}
              <b-icon icon="chevron-down" v-if="index!=$route.matched.length-1" scale="0.5"></b-icon>
              <b-popover v-if="index<$route.matched.length-1" container="popover-layout" :target="`bc-nav-item-${index}`" triggers="hover" placement="bottom-right">
                <b-nav vertical>
                  <b-nav-item :title="item.name" :key="item.name+index" v-for="(item,index) of getChildrenRoutes(r)" :to="item.path" class="p-0 rounded-0">
